...
exports.rajouteAppointment = async (req ,res) => {
    try{
        ...
        console.log(calendarId +' '+titre+' '+date_debut+' '+date_fin +' '+ userId);

        if(!calendarId||!titre||!date_debut||!date_fin ){

            console.log(calendarId +' '+titre+' '+date_debut+' '+date_fin);
            return res.status(400).json({error: 'manque d\'inforamtion pour créer un rdv'});
        }
        ...
        if(start >= end){
            return res.status(400).json({error :'date de debut doit etre avent la date de fin !'})
        }
...
exports.updateAppointment = async (req, res) => {
    try {
        ...
        const updatedRdv = await model.updateAppointment(id_rdv, { titre, date_debut, date_fin, description });
        if (!updatedRdv) {
            return res.status(404).json({ error: "RDV pas trouvé" });
        }

        return res.status(200).json({ message: "RDV mis à jour avec succès", rdv: updatedRdv });
    } catch (error) {
